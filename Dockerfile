...
#install extra tools
RUN sudo dnf install python39 librevenge-gdb git clang-tools-extra perf wget -y
RUN sudo dnf install ninja-build -y
RUN sudo dnf install vtk-devel openmpi-devel -y
RUN sudo dnf install mpfr-devel boost-devel tbb-devel gmp-devel -y
RUN wget https://github.com/CGAL/cgal/releases/download/v5.5.2/CGAL-5.5.2.tar.xz -O /tmp/CGAL-5.5.2.tar.xz
RUN tar xf /tmp/CGAL-5.5.2.tar.xz -C /tmp
RUN cd /tmp/CGAL-5.5.2 && cmake . && make && sudo make install
